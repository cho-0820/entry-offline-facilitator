import difflib

def clean_lines(text):
    if not text:
        return []
    # Strip leading/trailing whitespaces from each line and remove empty lines
    return [line.strip() for line in text.splitlines() if line.strip()]

def extract_block_specs(content):
    start_idx = content.find("const BLOCK_SPECS")
    if start_idx == -1:
        return ""
    # Find match for BLOCK_SPECS
    end_idx = content.find("};", start_idx) + 2
    return content[start_idx:end_idx].strip()

def extract_validator(content):
    start_idx = content.find("function validateBlockJsonTypes")
    if start_idx == -1:
        return ""
    
    # Locate checkBlock(codeJson) call near the end of the validator function
    end_idx = content.find("return checkBlock(codeJson);", start_idx)
    if end_idx == -1:
        end_idx = content.find("return checkBlock(code_json);", start_idx)
    
    if end_idx != -1:
        # Find closing brace after that return statement
        close_brace_idx = content.find("}", end_idx)
        if close_brace_idx != -1:
            return content[start_idx:close_brace_idx + 1].strip()
    return ""

def extract_prompt(content):
    start_idx = content.find("const systemPrompt = `")
    if start_idx == -1:
        return ""
    end_idx = content.find("`;", start_idx)
    return content[start_idx:end_idx + 2].strip()

def main():
    file_orig = r"c:\Users\ohmyg\OneDrive\Desktop\AI facilitator\entry-offline\src\main\ipcMainHelper.ts"
    file_port = r"c:\Users\ohmyg\OneDrive\Desktop\AI facilitator\facilitator-api\app\api\chat\route.ts"
    
    with open(file_orig, 'r', encoding='utf-8') as f:
        orig = f.read()
    with open(file_port, 'r', encoding='utf-8') as f:
        port = f.read()
        
    orig_specs = clean_lines(extract_block_specs(orig))
    port_specs = clean_lines(extract_block_specs(port))
    
    orig_val = clean_lines(extract_validator(orig))
    port_val = clean_lines(extract_validator(port))
    
    orig_prompt = clean_lines(extract_prompt(orig))
    port_prompt = clean_lines(extract_prompt(port))
    
    print("=== BLOCK_SPECS Diff ===")
    if orig_specs == port_specs:
        print("100% Identical")
    else:
        diff = difflib.unified_diff(orig_specs, port_specs, lineterm='')
        print('\n'.join(diff))
        
    print("\n=== validateBlockJsonTypes Diff ===")
    if orig_val == port_val:
        print("100% Identical")
    else:
        diff = difflib.unified_diff(orig_val, port_val, lineterm='')
        print('\n'.join(diff))
        
    print("\n=== systemPrompt Diff ===")
    if orig_prompt == port_prompt:
        print("100% Identical")
    else:
        diff = difflib.unified_diff(orig_prompt, port_prompt, lineterm='')
        print('\n'.join(diff))

if __name__ == "__main__":
    main()
